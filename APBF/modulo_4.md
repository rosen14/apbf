...
- [Semana05_CVAE_CERN.ipynb](./teorico_practicos/Semana05_CVAE_CERN.ipynb)
...
$$
ELBO_{\phi,\theta}= \mathbb E_{\vec{z}\sim q_\phi(\vec{z}|\vec{x})}\left[\ln q_\theta(\vec{x}|\vec{z})\right] - \mathbb E_{\vec{z}\sim q_\phi(\vec{z}|\vec{x})}\left[\ln q_\phi(\vec{z}|\vec{x})-\ln p(\vec{z})\right].
$$
